(ns marketentry.marketentryllm
  ...
  the MAR public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-ma-entity?=" (:has-ma-entity? e)
                        " ice-verified?=" (:ice-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-ma-entity? e))
                              (:has-ma-entity? e))
                          (or (not (:requires-ice? e))
                              (:ice-verified? e)))
                   0.9 0.3)}))
