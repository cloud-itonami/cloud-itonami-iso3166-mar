(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Morocco-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Morocco-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-ma-entity?
                                       true` (near-universal for Morocco
                                       ...
                                       Grounded in Moroccan legal entity
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(ICE/統一資格/marchespublics.gov.ma登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- ma-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-ma-entity? true`, INDEPENDENTLY verify
  `:has-ma-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-ma-entity?` ground truth (most Morocco public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-ma-entity? e))
                 (not (true? (:has-ma-entity? e))))
        [{:rule :ma-entity-missing
          :detail (str subject " はモロッコ居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- ice-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-ice? true`, INDEPENDENTLY check
  `:ice-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-ice? e))
                 (not (true? (:ice-verified? e))))
        [{:rule :ice-unverified
          :detail (str subject " はICE確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (ma-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (ice-unverified-violations request st)
