...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (MAR, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up ma-entity-missing) ==")
    ...
    (println "== filing/submit eng-4 (ma-entity-missing -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up ice-unverified) ==")
    ...
    (println "== filing/submit eng-5 (ice-unverified -> HARD hold) ==")
