# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Morocco

## Classification

- Repository: `cloud-itonami-iso3166-mar`
- ISO 3166: `MAR` (Morocco)
...
- Social impact: [:moroccan-sme-market-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on a Moroccan
  public contract
...
- registration walkthrough for the national public-tendering portal
  marchespublics.gov.ma, managed by the Moroccan treasury (Trésorerie
  Générale du Royaume), where government tenders are published
- business/tax registration checklist: a Registre du Commerce (RC) entry
  and an ICE (Identifiant Commun de l'Entreprise) number via OMPIC
  (Office Marocain de la Propriété Industrielle et Commerciale)
- local-content / preferential-procurement navigation: Morocco's public
  procurement code preference margins for national SMEs and
  domestically manufactured goods on qualifying tenders
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Moroccan-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Morocco).
