...
2. Register the client's intake: business type, target public function,
   prior filing history in Morocco if any.
3. Run the advisor in read-only mode against marchespublics.gov.ma, the
   national public-tendering portal.
4. Compare the checklist against the client's current documentation
   (OMPIC Registre du Commerce entry, ICE number).
...
- named referral relationship with Moroccan-licensed counsel or a
...
a working referral relationship with Moroccan-licensed counsel or a
registered agent for whatever licensed representation the law of
Morocco requires for actual public-procurement filings.
